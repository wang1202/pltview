...
void python_script_callback(Widget w, XtPointer client_data, XtPointer call_data);
void pyscript_close_callback(Widget w, XtPointer client_data, XtPointer call_data);
char *generate_python_script(PlotfileData *pf);
int save_script_to_file(const char *script, const char *filename);
...
void init_gui(PlotfileData *pf, int argc, char **argv) {
    ...
    /* Python script button - generates matplotlib code for current view */
    n = 0;
    XtSetArg(args[n], XtNlabel, "PyScript"); n++;
    button = XtCreateManagedWidget("pyscript", commandWidgetClass, tools_box, args, n);
    XtAddCallback(button, XtNcallback, python_script_callback, NULL);
...
/* Global for pyscript dialog */
static Widget pyscript_dialog_shell = NULL;

/* Save script to file, returns 1 on success */
int save_script_to_file(const char *script, const char *filename) {
...
/* Close callback for pyscript dialog */
void pyscript_close_callback(Widget w, XtPointer client_data, XtPointer call_data) {
    (void)w; (void)client_data; (void)call_data;
    if (pyscript_dialog_shell) {
        XtPopdown(pyscript_dialog_shell);
        XtDestroyWidget(pyscript_dialog_shell);
        pyscript_dialog_shell = NULL;
    }
}

/* Generate Python script to recreate current visualization */
char *generate_python_script(PlotfileData *pf) {
    /* Colormap name lookup */
    const char *cmap_names[] = {"viridis", "jet", "turbo", "plasma", "hot", "cool", "gray", "magma"};
    const char *cmap_name = (pf->colormap >= 0 && pf->colormap < 8) ? cmap_names[pf->colormap] : "viridis";

    /* Determine axis labels based on slice axis */
    const char *axis_labels[] = {"X", "Y", "Z"};
    int x_axis, y_axis;
    double extent[4];  /* xmin, xmax, ymin, ymax */

    switch (pf->slice_axis) {
        case 0:  /* X slice: show Y vs Z */
            x_axis = 1; y_axis = 2;
            extent[0] = pf->prob_lo[1]; extent[1] = pf->prob_hi[1];
            extent[2] = pf->prob_lo[2]; extent[3] = pf->prob_hi[2];
            break;
        case 1:  /* Y slice: show X vs Z */
            x_axis = 0; y_axis = 2;
            extent[0] = pf->prob_lo[0]; extent[1] = pf->prob_hi[0];
            extent[2] = pf->prob_lo[2]; extent[3] = pf->prob_hi[2];
            break;
        case 2:  /* Z slice: show X vs Y */
        default:
            x_axis = 0; y_axis = 1;
            extent[0] = pf->prob_lo[0]; extent[1] = pf->prob_hi[0];
            extent[2] = pf->prob_lo[1]; extent[3] = pf->prob_hi[1];
            break;
    }

    /* Unit string - extent values are raw coordinates from plotfile (typically meters) */
    const char *unit_str = "(m)";

    /* Allocate buffer for script */
    char *script = malloc(8192);
    if (!script) return NULL;

    /* Build the script */
    snprintf(script, 8192,
        "#!/usr/bin/env python3\n"
        "\"\"\"\n"
        "Generated by pltview - recreate this visualization in Python\n"
        "Plotfile: %s\n"
        "Variable: %s\n"
        "\"\"\"\n"
        "import numpy as np\n"
        ...
        "\n"
        "# === Configuration from pltview ===\n"
        "plotfile_path = \"%s\"\n"
        "variable = \"%s\"\n"
        "slice_axis = %d      # 0=X, 1=Y, 2=Z\n"
        "slice_index = %d\n"
        "level = %d           # AMR level\n"
        "colormap = \"%s\"\n"
        "vmin, vmax = %.6g, %.6g\n"
        "extent = [%.6g, %.6g, %.6g, %.6g]  # [xmin, xmax, ymin, ymax]\n"
        "xlabel, ylabel = \"%s %s\", \"%s %s\"\n"
        "\n"
        "# === Option 1: Load with yt ===\n"
        "# import yt\n"
        "# ds = yt.load(plotfile_path)\n"
        "# slc = ds.slice([\"x\",\"y\",\"z\"][slice_axis], slice_index)\n"
        "# frb = slc.to_frb(width=ds.domain_width[[0,1,2]!=slice_axis],\n"
        "#                  resolution=(512,512))\n"
        "# data = np.array(frb[variable])\n"
        "\n"
        "# === Option 2: Load with pyamrex ===\n"
        "# import amrex.space3d as amr\n"
        "# pf = amr.PlotFile(plotfile_path)\n"
        "# mf = pf.get_level(level)\n"
        "# data = mf.to_numpy(variable)\n"
        "# # Extract 2D slice from 3D data\n"
        "# if slice_axis == 0: data = data[slice_index, :, :]\n"
        "# elif slice_axis == 1: data = data[:, slice_index, :]\n"
        "# else: data = data[:, :, slice_index]\n"
        "\n"
        "# === Placeholder: Replace with your data loading ===\n"
        "data = np.random.rand(100, 100)  # Replace with actual data\n"
        "\n"
        "# === Create plot ===\n"
        "fig, ax = plt.subplots(figsize=(10, 8))\n"
        "im = ax.imshow(data, origin='lower', cmap=colormap,\n"
        "               vmin=vmin, vmax=vmax, extent=extent, aspect='auto')\n"
        "ax.set_xlabel(xlabel)\n"
        "ax.set_ylabel(ylabel)\n"
        "cbar = plt.colorbar(im, ax=ax)\n"
        "cbar.set_label(f'{variable}')\n"
        "ax.set_title(f'{variable} (slice {slice_index})')\n"
        "plt.tight_layout()\n"
        "plt.show()\n",
        pf->plotfile_dir,
        pf->variables[pf->current_var],
        ...
        axis_labels[x_axis], unit_str,
        axis_labels[y_axis], unit_str
    );

    return script;
}

/* Callback for PyScript button */
void python_script_callback(Widget w, XtPointer client_data, XtPointer call_data) {
    (void)w; (void)client_data; (void)call_data;
    Arg args[10];
    int n;
    ...
    char msg[512];
    static char saved_filename[MAX_PATH];
    ...
    /* Generate the script */
    char *script = generate_python_script(global_pf);
    if (!script) return;

    /* Create filename based on plotfile name */
    const char *basename = strrchr(global_pf->plotfile_dir, '/');
    basename = basename ? basename + 1 : global_pf->plotfile_dir;
    snprintf(saved_filename, sizeof(saved_filename), "pltview_%s.py", basename);

    /* Save to file */
    int success = save_script_to_file(script, saved_filename);
    free(script);

    /* Create popup dialog */
    n = 0;
    XtSetArg(args[n], XtNtitle, "Python Script"); n++;
    pyscript_dialog_shell = XtCreatePopupShell("pyscriptDialog", transientShellWidgetClass, toplevel, args, n);

    n = 0;
    form = XtCreateManagedWidget("form", formWidgetClass, pyscript_dialog_shell, args, n);

    /* Message label */
    if (success) {
        snprintf(msg, sizeof(msg), "Python script saved to:\n%s", saved_filename);
    } else {
        snprintf(msg, sizeof(msg), "Error: Could not save script");
    }
    ...
    label = XtCreateManagedWidget("msgLabel", labelWidgetClass, form, args, n);
    ...
    button = XtCreateManagedWidget("okButton", commandWidgetClass, form, args, n);
    XtAddCallback(button, XtNcallback, pyscript_close_callback, NULL);

    /* Show dialog */
    XtPopup(pyscript_dialog_shell, XtGrabNone);
}
